...
Deploy the full stack (Ollama + Cognee + Streamlit) to a DO GPU droplet with Docker Compose.

### What runs where

| Component | Runs as | Notes |
|---|---|---|
| **Ollama** (LLM + embeddings) | Docker container with GPU | Hosts `nomic-embed-text` + `cognee-distillabs-model-gguf-quantized` |
| **Cognee app + Streamlit** | Docker container | Port 8501, queries Qdrant Cloud + Ollama |
| **Qdrant** | Qdrant Cloud (hosted) | Already loaded with 14,837 vectors across 6 collections |

### Prerequisites

- A DO GPU Droplet (NVIDIA GPU, Ubuntu)
- Docker + Docker Compose + NVIDIA Container Toolkit installed
- A [Qdrant Cloud](https://cloud.qdrant.io) cluster with snapshots restored

### Files you need to manually upload (gitignored)

| Item | Size | Description |
|---|---|---|
| `models/` | ~2.7GB | GGUF model files — copy from USB |
| `cognee_export/` | ~462MB | Pre-built knowledge graph + data files — copy from USB |
| `.env` | tiny | Qdrant Cloud credentials — create from `.env.example.deploy` |
...
cd ai-memory-hackathon

# 2. Upload the large files (from your local machine)
# Option A: scp from local
scp -r models/ root@<droplet-ip>:~/ai-memory-hackathon/
scp -r cognee_export/ root@<droplet-ip>:~/ai-memory-hackathon/

# Option B: if files are on DO Spaces / S3
# wget or rclone them down

# 3. Create .env with your Qdrant Cloud credentials
...
# 4. Validate everything is in place
...
### Restore Qdrant snapshots (one-time, from local machine)

If the Qdrant Cloud cluster doesn't have the data yet, restore from the snapshot files:

```bash
# Download snapshots
python download-from-spaces.py

# Restore to Qdrant Cloud (set QDRANT_URL and QDRANT_API_KEY in .env first)
python restore-snapshots.py
```
...
├── ollama          (ollama/ollama:latest, GPU, port 11434)
│   └── models/     (mounted read-only)
├── ollama-init     (one-shot: registers GGUF models with Ollama)
└── app             (Dockerfile.app, port 8501)
    ├── cognee_export/   (baked into image)
    ├── streamlit_app.py (Streamlit UI)
    └── → Qdrant Cloud   (remote, via QDRANT_URL)
```
...
docker compose -f docker-compose.deploy.yml logs -f ollama
...
- **Ollama not starting**: Check GPU drivers — `nvidia-smi` should work on the host
- **Model registration fails**: Check `docker compose logs ollama-init` — models may already be registered
- **Streamlit hangs**: Check `docker compose logs app` — usually waiting for Ollama or Qdrant timeout
- **Qdrant timeout on Entity_name**: This is the largest collection (8,816 vectors). The app handles timeouts gracefully and still returns answers.
